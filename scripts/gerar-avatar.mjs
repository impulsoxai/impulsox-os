 ...
 * ImpulsoX AI. Modelo: OmniHuman v1.5 (ByteDance) via Fal.ai ou kie.ai.
 *
 * Uso:
 ...
 *     [--resolucao 1080p|720p] [--provedor fal|kie] [--dry-run]
 *
 ...
 *   - FAL_KEY/KIE_KEY nunca aparece em log ou erro
 */
...
const KIE_MODEL_AVATAR = { omnihuman: "omnihuman-1.5", "kling-avatar": "kling/ai-avatar-pro" };
...
if (dryRun) {
  console.log(JSON.stringify({
    dry_run: true, foto, audio, saida, resolucao, modelo, provedor,
    endpoint: provedor === "fal" ? MODELO : KIE_MODEL_AVATAR[modelo],
...
registrarPasso({ skill: "/post", etapa: `gerando avatar (${modelo}/${provedor}) · ${resolucao}`, status: "inicio" });
...
if (provedor === "kie") {
  ...
  try {
    ...
    registrarCusto({ script: "gerar-avatar", modelo: `${modelo}(kie)`, custo: 0 });
